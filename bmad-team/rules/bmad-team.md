...
7. **Documentation Maintenance**: Documentation is split strictly. See @docs-standard.md and @documentation-hierarchy.md.
    ...
    - **Shared** (`docs/`): System Architecture Map (`docs/architecture_map.md`), Product Briefs in `docs/briefs/` (or `docs/product_brief.md` directly under the `docs/` folder, which must always be respected if present), API JSON contracts (`docs/api_contract.md` directly under the `docs/` folder, which must always be respected if present), PRDs in `docs/prd/`, and LLDs in `docs/lld/`. **ALWAYS** version in git. No credentials.
