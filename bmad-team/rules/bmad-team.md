...
7. **Documentation Maintenance**: Documentation is strictly kept in the git-tracked `docs/` directory. No local folders like `agent_docs/` are required.
    ...
    - **Requirements First**: Project PRD and LLD in `docs/` must be approved before feature implementation begins, unless **Spike Mode** is active.
...
10. **Task Planning Mandate**: NEVER start implementation or significant research without a valid Feature Specification (`docs/features/{NNN}_{feature_name}_spec.md`) and a local `task.md` checklist. If missing, you MUST run **Phase 0: Planning** to create the specification.
