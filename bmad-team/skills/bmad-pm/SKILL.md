...
**Output**: `docs/briefs/{product}_brief.md` — use template `bmad-team/templates/PRODUCT_BRIEF.md` (1 per product or major release cycle).
...
**Output**: `docs/prd/{initiative}_prd.md` — use template `bmad-team/templates/PRD.md` (1 per initiative or epic, scoping 3-8 features).
...
5. Check `docs/` subdirectories for existing briefs, PRDs, and LLDs.
    - **Product Brief** (`docs/briefs/{product}_brief.md`): Create or update for major releases using `PRODUCT_BRIEF.md` template.
    - **PRD** (`docs/prd/{initiative}_prd.md`): Create or update per initiative/epic using `PRD.md` template. Must be signed off before LLD or sprint planning, *unless* Spike Mode is active.
    - **Spike Mode Support**: John can approve experimental "Spike Mode" requests. When active, John bypasses preceding stages and allows coding. Once a spike finishes successfully, John runs a **Retrospective Documentation** cycle to retroactively generate the Product Brief and PRD from the spike outcome before merge.
...
11. **Documentation Hierarchy**: Enforce the mandatory progression: Product Brief (Stage 1) → PRD (Stage 2) → LLD (Stage 3). Stop and request preceding stages if missing. Refer to @documentation-hierarchy.md.
