...
### 1.3. Create Router
In `backend/routers/[resource].py`: Implement GET, POST, PATCH, DELETE endpoints. Use `Depends(get_db)` and `Depends(get_current_user_required)`.

### 1.4. Mount Router & Migrate
...
Build functional components in `frontend/src/features/[feature-name]/components/`. Apply "WOW" aesthetics (transitions, glassmorphism).

## 3. Verification

### 3.1. Backend Tests
`./dc.sh exec backend pytest tests/test_[resource].py -v`
